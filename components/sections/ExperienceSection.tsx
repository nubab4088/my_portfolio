...
export default function ExperienceSection() {
  ...
  return (
    <section id="experience" className="bg-[#0a0a0a] pt-6 pb-24 relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute top-1/2 right-10 -z-10 h-80 w-80 rounded-full bg-teal-500/10 blur-[120px]" />
      ...
      <div className="container-max mx-auto max-w-7xl px-6 relative z-10">
        ...
        >
          ...
          <motion.h2 variants={itemVariants} className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tight text-white leading-tight">
            Professional <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-white/90 to-teal-300">Journey</span>
          </motion.h2>
        </motion.div>
        ...
        >
          {experiences.map((exp, idx) => (
            <motion.div
              ...
              className="group relative rounded-[2rem] border border-white/10 bg-white/[0.02] p-8 md:p-12 backdrop-blur-xl transition-all duration-500 hover:border-teal-500/40 hover:bg-white/[0.04] shadow-2xl overflow-hidden"
            >
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 mb-10">
                {exp.description.map((desc, i) => (
                  <li key={i} className="text-sm sm:text-base text-[#a0a0a0] flex items-start gap-3 leading-relaxed">
                    <span className="h-1.5 w-1.5 rounded-full bg-teal-500 mt-2.5 shrink-0 group-hover:scale-125 transition-transform" />
                    <span>{desc}</span>
                  </li>
                ))}
              </ul>
              ...
              <div className="pt-6 border-t border-white/5 flex flex-wrap items-center gap-2">
                ...
                {exp.skills.map((skill) => (
                  <span
                    key={skill}
                    className="rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs font-medium text-[#e5e5e5] transition-colors hover:border-teal-500/50 hover:bg-teal-500/10 hover:text-white"
                  >
                    {skill}
                  </span>
                ))}
